...
class RF100Benchmark:
            ...
            for line in file:
                try:
                    ...
                    if not Path(proj_version).exists():
                        self.rf.workspace(workspace).project(project).version(version).download("YOLO")
                    else:
                        LOGGER.info("Dataset already downloaded.")
                    self.ds_cfg_list.append(Path.cwd() / proj_version / "LGPSD-DET.yaml")
                except Exception:
                    continue
    ...
    def evaluate(self, yaml_path: str, val_log_file: str, eval_log_file: str, list_ind: int):
        ...
        Examples:
            ...
            >>> benchmark = RF100Benchmark()
            >>> benchmark.evaluate("path/to/LGPSD-DET.yaml", "path/to/val_log.txt", "path/to/eval_log.txt", 0)
        """
        skip_symbols = ["🚀", "⚠️", "💡", "❌"]
