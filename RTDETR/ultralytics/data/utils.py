...
class HUBDatasetStats:
    ...
    Args:
        path (str): Path to LGPSD-DET.yaml or data.zip (with LGPSD-DET.yaml inside data.zip). Default is 'coco128.yaml'.
    ...
    def _unzip(path):
        """Unzip data.zip."""
        if not str(path).endswith('.zip'):  # path is LGPSD-DET.yaml
