...
if __name__ == '__main__':
    ...
    result = model.val(data='RTDETR/LGPSD-DET.yaml',
                      split='test',
                      imgsz=640,
                      batch=32,
    ...
    if model.task == 'detect':
        ...
        model_info_table.field_names = ["GFLOPs", "Parameters", "Pre-processing time/one picture", "Reasoning time/one picture", "Post-processing time/one picture", "FPS(Pre-processing + model reasoning + post-processing)", "FPS(reasoning)", "Model File Size"]
        ...
        print('-'*20, f'Result saved to{result.save_dir}/paper_data.txt...', '-'*20)
        print('-'*20, f'Result saved to{result.save_dir}/paper_data.txt...', '-'*20)
        print('-'*20, f'Result saved to{result.save_dir}/paper_data.txt...', '-'*20)
        print('-'*20, f'Result saved to{result.save_dir}/paper_data.txt...', '-'*20)
        print('-'*20, f'Result saved to{result.save_dir}/paper_data.txt...', '-'*20)